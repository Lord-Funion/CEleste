...
const CatalogEntry *active_entry() { return is_active && active_catalog < count ? &catalog[active_catalog] : nullptr; }
...
bool restart_entry() {
    if(!is_active || active_catalog >= count) {
        set_error("no active custom level");
        return false;
    }
